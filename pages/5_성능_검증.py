...
try:
    ...
    if font_path: plt.rc("font", family="NanumGothic")
...
if 'metrics' not in st.session_state: st.session_state.metrics = [{"name": "Accuracy", "value": 0.95}]
def add_metric(): st.session_state.metrics.append({"name": "", "value": 0.0})
def remove_metric(index): st.session_state.metrics.pop(index)
...
if st.button("🤖 AI로 성능 평가 리포트 생성하기", type="primary", use_container_width=True):
    ...
    if not metrics_dict: st.error("하나 이상의 유효한 성능 지표를 입력해주세요.")
    else:
        with st.spinner("Gemini 에이전트가 성능을 분석하고 리포트를 작성하고 있습니다..."):
            ...
            st.session_state['current_perf_metrics'] = metrics_dict
            st.rerun()
...
if 'generated_perf_report' in st.session_state:
    ...
    with col1:
        st.session_state['generated_perf_report'] = st.text_area("내용을 검토/수정하거나, 아래 AI 도구를 사용해 보세요.", value=st.session_state.generated_perf_report, height=400, key="perf_report_editor")
    with col2:
        ...
        if metrics_to_plot:
            ...
            ax.set_ylabel('Score'); ax.set_title('Performance Metrics'); ax.tick_params(axis='x', rotation=45)
            st.pyplot(fig)
    st.markdown("---")
    st.write("🤖 **AI 편집 도구모음**")
    ...
    if st.button("💾 이 최종 리포트를 이력으로 저장하기", type="primary", use_container_width=True):
        ...
        save_artifact(project_id=selected_id, stage="VERIFICATION", type="PERF_REPORT", content=full_content)
